...
const navLinkPlanCustom = document.querySelectorAll(".nav-link-plan__custom");
...
$(function () {
    ...
    if ($('.nav-link-plan__custom').length) {
        for (let i = 0; i < navLinkPlanCustom.length; i++) {

            if (navLinkPlanCustom[1].classList.contains('active')) {
...
/* header fixed sticy animations */

$(document).ready(function() {
  $(window).scroll(function() {
    if ($(document).scrollTop() > 50) {
      $(".navbar-gen-custom.navbar-expand-lg.navbar-dark").addClass("navbar-stuck");
    } else {
      $(".navbar-gen-custom.navbar-expand-lg.navbar-dark").removeClass("navbar-stuck");
    }
  });
});