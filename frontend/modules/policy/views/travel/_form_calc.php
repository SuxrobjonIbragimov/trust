                            ...
                            <?php if ($model->is_family): ?>
                                ...
                                <div class="container-items-parent"><!-- widgetContainer -->
                                    <?php foreach ($modelParentTravellers as $i => $modelItem): ?>
                                        <div class="item-parent panel panel-default my-2"><!-- widgetBody -->
...
//                                                    echo Html::activeHiddenInput($modelItem, "[{$i}]id");
                                                    echo $form->field($modelItem, "[{$i}]id")
                                                ...
                                                <div class="row custom-row--sm">
                                                    <div class="col-sm-12 col-12 field">
                                                        <?= $form->field($modelItem, "[{$i}]address", ['options' => ['class' => 'form-group']])->textInput([
                                                            'maxlength' => true,
                                                            'type' => 'hidden',
                                                            'class' => 'field__input form-control latin_letters_no_number',
                                                            'oninput' => 'this.value = this.value.toUpperCase()',
                        ...
                        success: function (response)
                        {
                            ...
                            if (response.ERROR > 0) {
                                ...
                                if (driver_id == null) {
                                    ...
                                    if (data.ADDRESS && param === 'travelparent') {
                                        $('#policy'+param+'traveller-'+driver_id+'-address').val(data.ADDRESS).parent('div').removeClass('has-error').find('.help-block').html('');
                                    } else if(param === 'travelparent') {
                                        $('#policy'+param+'traveller-'+driver_id+'-address').attr('type','text').parent('div').removeClass('has-error').find('.help-block').html('');
                                    }
                                }
                            }
                            isLoading = false;
